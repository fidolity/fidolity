...
export default function ProductDetailPage() {
  ...
  const handleExecute = async () => {
    ...
    try {
      ...
      setResult('Payment successful! Executing agent...');

      await new Promise(resolve => setTimeout(resolve, 1500));

      await fetch(
        `${import.meta.env.VITE_SUPABASE_URL}/rest/v1/marketplace_purchases?id=eq.${purchase.id}`,
        {
          method: 'PATCH',
          headers: {
            'apikey': import.meta.env.VITE_SUPABASE_ANON_KEY,
      ...
      await fetch(
        `${import.meta.env.VITE_SUPABASE_URL}/rest/v1/marketplace_items?id=eq.${item.id}`,
        {
          method: 'PATCH',
          headers: {
            'apikey': import.meta.env.VITE_SUPABASE_ANON_KEY,
          ...
          body: JSON.stringify({
            executions: item.executions + 1,
            revenue: parseFloat((item.revenue + item.price).toFixed(2))
          }),
        }
      );
  ...
  return (
    ...
    <div className="min-h-screen bg-transparent pt-32 pb-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <button
          onClick={() => navigate('/marketplace')}
          className="flex items-center gap-2 text-white hover:text-orange-500 mb-8 transition-colors"
        >
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start gap-6 mb-6">
            <div className="flex-1">
              ...
              <div className="flex items-center gap-2">
                <span className="px-3 py-1 bg-gray-800 text-gray-200 text-sm rounded-lg font-medium">
                  {item.category}
                </span>
                {item.x402_ready && (
        ...
        <div className="bg-black/30 backdrop-blur-sm rounded border border-gray-800-xl border border-gray-200 shadow-sm p-8 mb-6">
          ...
          <div className="flex flex-wrap gap-2 mb-4">
            ...
            <button
              ...
              className="ml-auto px-4 py-2 text-gray-300 hover:text-white bg-gray-800 border border-gray-700 rounded-lg transition-colors flex items-center gap-2 hover:bg-gray-700"
            >
